...
%=============================================================================%
% Turbolisp Runtime.
% This is separate from the Turbolisp module, which provides just an
% S-expression parser. Depending on your inputs, that might be enough :)
% Ideally we would also separate out the execute/reduce components, since you
% usually don't want both.
:- interface.
%=============================================================================%
...
:- include_module tl_runtime.builtin.
:- use_module tl_runtime.builtin.
...
builtin_bind(args("=", 4), mercury_bind(tl_runtime__builtin__builtin_eq_bind)).
builtin_bind(args("!", 4), mercury_bind(tl_runtime__builtin__builtin_ne_bind)).
builtin_bind(args("<", 4), mercury_bind(tl_runtime__builtin__builtin_lt_bind)).
builtin_bind(args(">", 4), mercury_bind(tl_runtime__builtin__builtin_gt_bind)).
builtin_bind(args("<=", 4), mercury_bind(tl_runtime__builtin__builtin_le_bind)).
builtin_bind(args(">=", 4), mercury_bind(tl_runtime__builtin__builtin_ge_bind)).

builtin_bind(args("+", 2), mercury_bind(tl_runtime__builtin__builtin_plus_bind)).
builtin_bind(args("-", 2), mercury_bind(tl_runtime__builtin__builtin_minus_bind)).
builtin_bind(args("*", 2), mercury_bind(tl_runtime__builtin__builtin_times_bind)).
builtin_bind(args("/", 2), mercury_bind(tl_runtime__builtin__builtin_divide_bind)).

builtin_bind(args("fn", 3), mercury_bind(tl_runtime__builtin__builtin_fn_bind)).
...
:- type preprocess_result --->
    ...
    comparison(tl_runtime.builtin.comparison, element, element, list(element)).
...
preprocess(Pred, In @ list([atom(Tag)|Tail]), Result, !Runtime) :-
    ...
    then
        ...
        tl_runtime.builtin.builtin_op_tag(Op, Tag),
        tl_runtime.builtin.comparison(Cmp) = Op
    then
...
reduce(Element, Result, !Runtime) :-
    ...
    ;
        ...
        ;
            ...
            tl_runtime.builtin.comparison_tag(Cmp, Tag),
            ...
            ;
                Tail = [Y|[N|[]]],
                tl_runtime.builtin.comparison(Cmp, A, B, CmpResult),
                (
                    CmpResult = tl_runtime.builtin.error(_),
                    Result = FallbackResult
                ;
                    (
                        CmpResult = tl_runtime.builtin.yes, Choice = Y
                    ;
                        CmpResult = tl_runtime.builtin.no, Choice = N
                    ),
        ...
        ;
            ...
            ( if
                ...
                tl_runtime__builtin__builtin_op_tag(
                    tl_runtime__builtin__define(Op), Tag)
            then
                (
                    Op = tl_runtime__builtin__fn,
                    ...
                    then
                        tl_runtime__builtin__builtin_fn_bind(Tail, _, !Runtime)
                    else
                        true
                    )
                ;
                    Op = tl_runtime__builtin__let
                ;
                    Op = tl_runtime__builtin__def
                ),
    ...
    ;
        ...
        ;
            ...
            ;
                ...
                tl_runtime.builtin.comparison(Cmp, A, B, CmpResult),
                (
                    CmpResult = tl_runtime.builtin.error(Error),
                    tl_runtime.builtin.comparison_tag(Cmp, Tag),
